...
public class GraphPathFinder {
    DefaultDirectedGraph<String, TickerEdge> graphCopy;
    List<String> baseAssetsList = Arrays.stream(BaseAssets.values()).map(BaseAssets::toString).toList();
    int maxDepth;

    public List<List<TickerEdge>> findAllPaths(int maxDepth) {
        graphCopy = GraphProvider.getCopy();

        this.maxDepth = maxDepth;
        List<List<TickerEdge>> paths = new ArrayList<>();
        List<TickerEdge> currentPath = new ArrayList<>();
    ...
    private void cyclicDfsEdges(List<List<TickerEdge>> paths, List<TickerEdge> currentPath,
        ...
        if (currentDepth > 0) {
            ...
            TickerEdge edge = graphCopy.getEdge(prevVertex, currentVertex);
            currentPath.add(edge);
        }
        if (cycleDetected && currentPath.size() <= maxDepth) {
            // Обнаружен цикл, добавляем текущий путь в paths
            List<TickerEdge> pathCopy = new ArrayList<>(currentPath);

            // Добавляем последнее ребро из currentVertex в начальный элемент в конец pathCopy
            TickerEdge edge = graphCopy.getEdge(currentVertex, visitedVertex.get(0));
            if (edge != null) {
                pathCopy.add(edge);
    ...
    private boolean arePathsEqual(List<TickerEdge> path1, List<TickerEdge> path2) {
